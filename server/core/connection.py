...
class ConnectionHandler:
    ...
    async def handle_connection(self, ws: websockets.ServerConnection):
        try:
            ...
            device_registry.unregister(self.device_id, self)
            try:
    ...
    def chat(self, query, depth=0):
        ...
        if depth == 0:
            ...
            # 唤醒提醒（baby-care 喂养过期）：一次性注入，
            # LLM 会在回复开头自然播报提醒（system 角色会被 dialogue 过滤，用 user+标记）
            pending_reminder = getattr(self, "pending_reminder", "")
            if pending_reminder:
                self.dialogue.put(Message(role="user", content=pending_reminder))
                self.pending_reminder = ""
                self.logger.bind(tag=TAG).info("已注入唤醒提醒到本次对话")
