...
async def handleHelloMessage(conn: "ConnectionHandler", msg_json):
    ...
    # 唤醒提醒：查询 baby-care 喂养状态，有过期的挂到 conn.pending_reminder，
    # 用户第一句对话时注入（chat 里处理）
    try:
        from core.handle.baby_care_reminder import fetch_baby_care_reminder
        asyncio.create_task(fetch_baby_care_reminder(conn))
    except Exception as e:
        conn.logger.bind(tag=TAG).debug(f"baby-care 提醒查询失败: {e}")
...
async def checkWakeupWords(conn: "ConnectionHandler", text):
    ...
    if not response or not response.get("file_path"):
        # 当前音色无缓存：现场用当前 TTS 合成，避免开机第一声回退到预置默认音色
        if await _wakeup_response_lock.acquire():
            try:
                response = await _generate_wakeup_response(conn, voice)
            finally:
...
async def _generate_wakeup_response(conn: "ConnectionHandler", voice: str) -> dict:
    """用当前 TTS 音色现场合成一条唤醒回应，写入缓存并返回"""
    if not conn.tts:
        return {}
    ...
    conn.logger.bind(tag=TAG).info(f"现场合成唤醒回应[{voice}]: {result}")
    return {"voice": voice, "file_path": file_path, "time": time.time(), "text": result}
...
async def wakeupWordsResponse(conn: "ConnectionHandler"):
    ...
    try:
        ...
        await _generate_wakeup_response(conn, voice)
    finally:
        # 确保在任何情况下都释放锁
