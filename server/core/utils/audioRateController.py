            ...
            if item_type == "message":
                ...
                while True:
                    ...
                    if elapsed_ms < output_ms:
                        ...
                        # 时间已到，跳出等待循环
                        # 诊断：追赶量超过 150ms 说明发生过停顿（生成慢/事件循环阻塞）
                        catchup = elapsed_ms - output_ms
                        if catchup > 150:
                            self.logger.bind(tag=TAG).warning(
                                f"⏱ 发送追赶 {catchup:.0f}ms（队列曾断供，播放位置 {output_ms}ms）"
                            )
                        break
                ...
                try:
                    ...
                    raise
                # 诊断：单帧发送（websocket 写）耗时超 50ms 说明网络/代理阻塞
                send_cost = (time.monotonic() - t_send) * 1000
                if send_cost > 50:
                    self.logger.bind(tag=TAG).warning(
                        f"⏱ 帧发送耗时 {send_cost:.0f}ms（websocket 写阻塞）"
                    )
