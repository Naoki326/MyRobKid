...
> 它仍然**完整保留**在「原始配置」只读页里（零字段丢失，不是
> 被删掉）。落差登记在此，不改表也不逐条重新裁量 —— 归属由规则机械复算，
> 本票只按表执行。
>
> （原文写的是「旧八组页面与原始配置只读页」。#31 收线删除了旧八组页面，
> 所以现在只剩后者——这条落差并没有因此变成「字段丢了」，因为旧页从来就
> 不是它被计入口径的落位，只是一个碰巧能看到它的地方。）
...
> **✅ 的口径**：「可盲测」指这十道题**可以用人工盲测验收**（域页结构让它们至多两次点击可达），不是「已由自动化验证」。父 spec 不引入浏览器测试基建，所以 `server/tests/test_config_topology_seam.py` **只钉结构前提**（每题的域归属 + 层位 + 该域有无常用层 → 推出的可见层级），**真实点击数仍是人工走查项**。两者合起来才构成 §10.2 的判据。
