...
**接入字段**:
引擎/插件卡片内让该组件跑起来的必填项（端点、凭据、模型与声音标识、协议/语言选择），卡内层归常用；裁决见 #16。
_Avoid_: 必填字段（不是同一概念）

**调优字段**:
引擎/插件卡片内组件跑起来之后的参数（阈值/速度/温度/切分…），卡内层归更多设置；与接入字段相对。
_Avoid_: 高级字段（本仓不用「高级」作层名）

**逃生口**:
配置页域结构之外的原始配置只读 JSON 视图（页脚工具），孤儿字段兑底；它不是侧栏域。裁决见 #16（六域改五域 + 逃生口）。
_Avoid_: 全部配置（旧页名）、高级（不是域）
