...
**会话性暂停** / **用户暂停**:
音乐让位的两种语义。**会话性暂停**由唤醒或新对话触发（用户要插话，不是要停音乐），答完静默数秒后自动续播；**用户暂停**由用户明确说「暂停」触发，必须说「继续」才续。二者必须分开记，否则用户说暂停后随便聊一句，音乐就自己回来了。状态上报里的两态字符串固定为 `paused_conversation` / `paused_user`。
...
**管线遥测**:
固件 MusicPlayer 每 2 秒打印的 `pipe:` 行（ring 水位/in_buf/下载字节/推帧与失败计数/位点 pos），音乐卡顿定位与位点核验的第一证据源；配套 USB 串口（115200）抓设备日志，`tools/serial_telemetry.py --assert` 可对位点做断言（不超墙钟、不低于起点、live 无数字位点）。
